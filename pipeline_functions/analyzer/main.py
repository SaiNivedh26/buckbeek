"""Eventarc adapter: forward finalized repository archives to private Cloud Run."""
...
def analyze_submission(cloud_event):
    ...
    if result_blob.exists():
        return
    # Eventarc is at-least-once. Creating this sentinel with a generation
    # precondition elects exactly one delivery before any expensive work starts.
    try:
    ...
    try:
        ...
        token = google.oauth2.id_token.fetch_id_token(auth_request, AGENT_URL)
        payload = {
