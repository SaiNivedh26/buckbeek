...
| Term | Meaning |
|---|---|
| **Pillar** | One of the five `clause.md` categories: Code Health, Test Coverage, CI/CD, Issue Management, Community. Each scores 0–10. |
| **Survey** | A file-count pass that runs before any model call. Walks the tree, sets yes/no flags (are there test files? a CI config? a README?). Costs nothing. |
| **Investigator** | An agent that explores the repo and writes notes. Never produces a score. One per pillar. |
| **Findings** | An investigator's notes, with file paths backing each claim. |
| **Scorer** | An agent that reads one investigator's findings plus the rubric text and returns 0–10. Never touches the repo. |
| **Clamp** | **A maximum score enforced by code, triggered by a countable fact.** Zero test files found → Test Coverage cannot exceed 2, whatever the scorer said. A speed limiter: it doesn't drive, it caps. Exists because a countable fact must beat a persuasive README. |
| **Abstain** | A pillar with too little evidence returns no score rather than guessing. Its weight is removed and the rest rescale. |
| **Budget** | Maximum tool calls a pillar may spend. 120 total, split across five pillars by weight and headroom. |
| **Evidence coverage** | 0–1, how much an investigator could actually determine. Below 0.25 the pillar abstains. |

A run is eight steps, six of them ordinary code:

```
1. Find repo, get commit SHA                              code
2. Survey: count files, set flags, split budget           code
3. Five investigators explore in parallel  -> Findings    agents
4. Save findings to SQLite                                code
5. Five scorers: findings + rubric -> scores              agents
6. Apply clamps                                           code
7. Weights, abstention renormalization, total             code
8. Report                                                 code
```

The agents do exactly two jobs: look around (3) and judge against a rubric band (5).
...
One rule shapes this: **get one pillar working end to end before writing the other four.**
Otherwise five investigator briefs get written, all wrong in the same way, and rewritten five
times.

| # | Build | Verify before moving on |
|---|---|---|
| 1 | `pyproject.toml`, `config.py`, `config.toml` | `gitcrawl --version` runs |
| 2 | `source.py` | correct SHA for a local path; a URL clones once and reuses |
| 3 | `survey.py`, `tools/filters.py` | vendored-heavy fixture: right exclusion count, no junk path survives |
| 4 | `storage/` — schema, save/load | findings round-trip |
| 5 | **Smoke test** ⚠️ one Agno Agent, Mistral, tool + `output_schema` | valid typed object after a tool call, ten runs in a row |
| 6 | `tools/repo_tools.py`, `budget.py`, `wrapping.py` | `../` blocked; missing file returns a marker not content; call 11 of a 10-call budget refuses |
| 7 | `agents/investigators.py` — Test Coverage only | finds the right files, stops with `stopped_reason="confident"` |
| 8 | `agents/scorers.py`, `rubric.py` | no-tests fixture scores low, well-tested fixture scores high |
| 9 | `scoring/clamps.py`, `aggregate.py` | reproduces `clause.md` §2.2 → **7.15**; renormalization with 1 and 2 abstentions |
| 10 | `orchestrator.py`, `report.py` | `gitcrawl .` prints a report: one pillar scored, four abstained |
| 11 | Remaining 4 investigator/scorer pairs, `asyncio.gather` | full run on this repo, then `psf/requests` |
| 12 | Injection + cost-scaling tests | score unmoved by a hostile README; large and small repos within 1 order of magnitude |

**Step 3 is shippable on its own** — `gitcrawl survey .` needs no model, no API key, and no
spend, so bulk screening works before a single agent exists.

**Step 5 is the riskiest assumption in the design**, which is why it runs before anything is
built on it. If tool use plus structured output is unreliable on Mistral, the fix (two calls
instead of one, or a different model tier) is cheap here and expensive after five
investigators exist.

**Step 10 is the milestone that matters.** Every architectural risk is retired by then — model
viability, tool safety, budget enforcement, arithmetic, storage. Step 11 is repetition.
